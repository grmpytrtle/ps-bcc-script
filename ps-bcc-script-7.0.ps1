...
Import-CSV -Path $csv -Header Name | ForEach-Object -Parallel {
    param ($backupDir)
...
} -ArgumentList $backupDir -ThrottleLimit 4  # Adjust the throttle limit as needed
...
Import-CSV -Path $csv -Header Name | ForEach-Object -Parallel {
    param ($browserName, $cachePaths, $userName)
    ...
    Clear-Cache -paths $ie_cachePaths -browserName "Edge/IE" -userName $_.Name

} -ArgumentList $backupDir -ThrottleLimit 4
